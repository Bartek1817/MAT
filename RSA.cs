...
namespace QuantumConsole
{
	...
	{	
		...
		*/
		public static int ModuleReverse(int a, int b) {
			int u = 1, w = a;
			int x = 0, z = b;
			int q; 
			while(w != 0) {
			...
			if(z == 1) {
				...
				Console.WriteLine("Znalezinono odwrtonosc modulo liczb {0} i {1} rowna {2}", a, b, x);
			}
			else {
				Console.WriteLine("Nie znaleziono odwrotnosci modulo.");
			}
			return x;
		}
		...
		public static void Main() {
			...
			int a = 9;		// Wiadomosc
			int b = (int) BigInteger.ModPow(a, c, N);	// Zakodowana wiadomosc
